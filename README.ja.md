...
複数のモデルを特殊な inputs 構造にまとめるローダー — rgthree Power Lora Loader のように
`inputs` が `{"on": bool, "lora": "名前", "strength": float, ...}` 形式の `lora_N` 項目を
多数持つもの — は、`extract_models()` 内の専用分岐（`_extract_power_loras`）で処理します。
