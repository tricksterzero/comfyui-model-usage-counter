...
表示言語は ComfyUI のロケール（`Comfy.Locale`）に追従します。既定は英語で、ロケールを
日本語にすると日本語で表示されます。ノード上にはこのように表示されます（生成を1回行うと現れます）。
...
`n日前` / `nヶ月前` / `n年前` で表示されます（全角を考慮して桁揃えされます）。日時は
ComfyUI を動かしている環境（サーバ）のタイムゾーンで表示します（ローカル運用ならお使いの
現地時刻と一致します）。
...
- `count` … そのモデルを使った回数
- `last_used` … 最後に使った日時（ComfyUI サーバ環境のローカル時刻、UTCオフセット付きの ISO 8601 形式）
...
ノード上の表示は、`execute()` が返す `ui` dict（集計を言語別に整形して持つ。`text`＝英語既定、
`text_en` / `text_ja`）を生成完了時の `executed` イベントで送り、同梱の JS 拡張
（`js/model_usage_counter.js`、`WEB_DIRECTORY` で配信）が ComfyUI のロケール（`Comfy.Locale`）に
応じた言語を選んで読み取り専用の複数行テキスト欄に描画することで実現しています。日時は
バックエンドで ComfyUI サーバ環境のタイムゾーンを使って整形します。
