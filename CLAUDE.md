...
- **コミットの分割方針**: その機能自体を説明するドキュメント（対応ローダー表など README の更新、
  CLAUDE.md の対応状況の記述）は **feat コミットに同梱**する。feat とは独立したドキュメント
  （運用ルール・プラクティス、機能と無関係なメモ等）だけを別の `docs` コミットに分ける。
  理由: 機能とその説明を1コミットで完結させ、「コードは対応済みなのに README は未対応のまま」
  という矛盾した中間状態のコミットを作らないため。
